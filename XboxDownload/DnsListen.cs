...
namespace XboxDownload
{
    class DnsListen
    {
        ...
        public static Regex regHosts = new Regex(@"^[a-zA-Z0-9][-a-zA-Z0-9]{0,62}(\.[a-zA-Z0-9][-a-zA-Z0-9]{0,62})+$");
                        ...
                        if (dns.QR == 0 && dns.Opcode == 0 && dns.Querys.Count == 1 && (dns.Querys[0].QueryType == QueryType.A || dns.Querys[0].QueryType == QueryType.AAAA))
                        {
                            if (dns.Querys[0].QueryType == QueryType.A)
                            {
                                ...
                                if (Properties.Settings.Default.EnableCdnIP)
                                {
                                    ...
                                    {
                                        argb = 0x0000FF;
                                        List<ResouceRecord> lsResouceRecord = lsIp.OrderBy(a => Guid.NewGuid()).Take(3).ToList();
                                        dns.QR = 1;
                                        dns.RA = 1;
                                        dns.RD = 1;
                                    ...
                                    foreach (var item in dicCdnHosts2)
                                    {
                                        if (item.Key.IsMatch(queryName))
                                        {
                                            ...
                                            argb = 0x0000FF;
                                            List<ResouceRecord> lsResouceRecord = item.Value.OrderBy(a => Guid.NewGuid()).Take(3).ToList();
                                            dns.QR = 1;
                                            dns.RA = 1;
                                            dns.RD = 1;
                                            dns.ResouceRecords = lsResouceRecord;
