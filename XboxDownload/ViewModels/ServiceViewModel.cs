...
public partial class ServiceViewModel : ObservableObject
{
    ...
    private async Task TestIPv6()
    {
        ...
        var fastestIp = await HttpClientHelper.GetFastestHttpsIpAsync(ips);
    ...
    public async Task ToggleListeningAsync()
    {
        ...
        else
        {
            var fieldNames = new[]
            {
                ...
                nameof(PsIp), nameof(NsIp), nameof(EaIp), nameof(BattleIp)
            };
    ...
    private async Task FastestAkamaiIpAsync()
    {
        ...
        if (IsFastestAkamaiIp)
        {
            ...
            if (IsListening)
            {
                if (bestIp != null)
                {
                    XboxGlobalIp = XboxCn1Ip = XboxCn2Ip = XboxAppIp = PsIp = NsIp = EaIp = BattleIp = bestIp.Ip;
